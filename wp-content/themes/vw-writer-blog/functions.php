...
function vw_writer_blog_activation_notice() {
	...
	echo '<h3>'. esc_html__( 'Warm Greetings to you!!', 'vw-writer-blog' ) .'</h3>';
...
function vw_writer_blog_scripts() {
	...
	wp_enqueue_script( 'vw-writer-blog-custom-scripts-jquery', get_template_directory_uri() . '/js/custom.js', array('jquery') );
...
function vw_writer_blog_ie_stylesheet(){
	wp_enqueue_style('vw-writer-blog-ie', get_template_directory_uri().'/css/ie.css');
	wp_style_add_data( 'vw-writer-blog-ie', 'conditional', 'IE' );
}
add_action('wp_enqueue_scripts','vw_writer_blog_ie_stylesheet');
